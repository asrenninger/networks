...
We can think about this philosophy of streets as the walking model—compared to, say, driving or briking. 
...
graph <-
  graph %>%
  activate(nodes) %>%
  mutate(between = betweenness(graph, v = V(graph), directed = FALSE),
...
p_tween <- 
  ...
  geom_sf(data = graph %>% 
            ...
            as_tibble() %>%
            st_as_sf() %>%
            mutate(group = factor(cut(as.numeric(between), natural_tween))),
          aes(colour = group, fill = group, size = between)) +
...
lab <- 
  c(round(rescale(natural_close, to = c(0, 10)), 2)) %>% 
  as_tibble() %>% 
  pull()
...
p_close <- 
  ...
  geom_sf(data = graph %>% 
            ...
            as_tibble %>%
            st_as_sf() %>%
            mutate(group = factor(cut(as.numeric(close), natural_close))),
          aes(colour = group, fill = group, size = close)) + 
  scale_fill_manual(values = pal,
                     labels = lab,
                     na.translate = FALSE,
                     name = "closeness",
...
lab <- 
  c(round(rescale(natural_degri, to = c(0, 10)), 2)) %>% 
  as_tibble() %>% 
  pull()
...
p_degri <- 
  ...
  geom_sf(data = graph %>% 
            ...
            as_tibble() %>%
            st_as_sf() %>% 
            mutate(group = factor(cut(as.numeric(degree), natural_degri))),
          aes(colour = group, fill = group, size = degree)) + 
  scale_fill_manual(values = pal,
                     labels = lab,
                     na.translate = FALSE,
                     name = "degree",
...
p_density <-
  ...
  summarise(n = n()) %>%
  ggplot() +
  geom_density(aes(n, fill = n), alpha = 0.75, colour = '#ffffff', fill = pal[1]) +
...
agg <- (p_tween | p_close) / (p_degri | p_density) & theme_bm_legend()
...
hoods <- 
  graph %>%
  activate(nodes) %>%
  mutate(cluster = factor(membership(communities))) %>%
  as_tibble() %>%
  group_by(cluster) %>%
  add_tally() %>%
  ungroup() %>%
...
cols <- sample(ramp(unique(hoods$cluster)))
...
p_labels <- 
  ...
  geom_sf(data = background,
          aes(), fill = '#353535', colour = NA, size = 0) +
  geom_sf(data = hoods,
          aes(colour = cluster, fill = cluster), size = 0.5) + 
  ...
  scale_color_manual(values = cols,
                     ...
                     name = "degree",
                     guide = 'none') +
  labs(title = "estimated demarcation", subtitle = "NIEGHBORHOODS") +
  theme_bm_legend()
...
p_sizes <- 
  ...
  labs(title = "associated size", subtitle = "NIEGHBORHOODS") +
  theme_bm_legend()

clusters <- p_labels | p_sizes

ggsave(clusters, filename = "clusters.png", height = 10, width = 18, dpi = 300)
