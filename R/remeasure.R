...
## the big one
get_metrics <- function(county, months) {
  
  print(county)
  
  ## palette 
  pal <- scico::scico(9, palette = 'tokyo')
  ...
  ## plotting it 
  print("viz")
  correlate(correlation, county, glue("viz/results/counties/correlations/correlation_{county}.png"))
  diversify(diversities, county, glue("viz/results/counties/diversities/diversity_{county}.png"))
  
  print("writing centrality")
  centrality %>% 
    mutate(city = county) %>% 
    write_csv(glue("data/processed/counties/centralities/centrality_{county}.csv"))
  ...
  correlation %>% 
    ...
    drop_na() %>% 
    mutate(city = county) %>%
    write_csv(glue("data/processed/counties/correlations/correlation_{county}.csv"))
  
  write_csv(diversities, glue("data/processed/counties/diversities/diversity_{county}.csv"))
  return(diversities)
...
correlate <- function(correlations, metro, name) {
  ...
  ggheatmap <- 
    ...
    scico::scale_colour_scico(palette = 'tokyo', direction = -1,
                              guide = 'none') +
